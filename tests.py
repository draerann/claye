...
def test_nombre_disques():
    plat = init(5)
    assertTrue(nombre_disques(plat,0),5,"tour vide")
    assertTrue(nombre_disques(plat,1),0, "tour remplie a 5")

def test_disque_supérieur():
    plat = init(5)
    assertTrue(disque_superieur(plat,1), -1, "tour vide")
    assertTrue(disque_superieur(plat,0), 5, "tour remplie")

def test_position_disque():
    plat = init(5)
    assertTrue(position_disque(plat,1),0,"position initiale")
    assertTrue(position_disque(plat,2),0, "pos initiale")

def test_verifier_deplacement():
    plat = init(5)
    assertTrue(verifier_deplacement(plat,2,0), False, "vide sur plein")
    assertTrue(verifier_deplacement(plat,1,2),False, "vide sur vide")
    assertTrue(verifier_deplacement(plat,0,2),True, "plein sur vide")
    assertTrue(verifier_deplacement(plat,0,0),False, "plein sur plein")
